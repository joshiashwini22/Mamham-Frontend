            ...
            <div className="overflow-y-auto max-h-60">
              {notifications.length > 0 ? (
                notifications.map((notification, index) => (
                  <div
                    key={index}
                    className={`block px-4 py-2 text-sm cursor-pointer ${notification.is_read ? 'text-gray-700' : 'text-gray-700 font-bold'} ${notification.is_read ? '' : 'bg-gray-200'}`} // Apply font-bold for unread notifications and gray background if not read
                    onClick={() => handleNotificationClick(notification.id)}
                  >
                    <span>{notification.message}</span>
                    {!notification.is_read && <span className="absolute top-0 right-0 inline-block w-2 h-2 bg-blue-500 rounded-full"></span>}
                  </div>
                ))
              ) : (
