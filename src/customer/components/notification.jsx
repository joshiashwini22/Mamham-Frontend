...
const NotificationDropdown = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await axios.get(
        `http://your-backend-api-url/notifications/${customerId}`
      );
      setNotifications(response.data);
      setShowDropdown(true); // Show the dropdown after fetching notifications
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchNotifications();
  }, []); // Empty dependency array to fetch notifications only once
